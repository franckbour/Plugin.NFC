...
namespace Plugin.NFC
{
    ...
    public class NFCImplementation : INFC
    {
        ...
        /// <param name="makeReadOnly">Make tag read-only</param>
        void WriteOrClearMessage(ITagInfo tagInfo, bool clearMessage = false, bool makeReadOnly = false)
        {
            try
            {
                if (_currentTag == null)
                ...
                if (clearMessage && tagInfo.IsFormatable)
                {
                    // if the tag is NdefFormatable then we format it into Ndef
                    FormatNdefFormatable(tagInfo);
                    return;
                }
        ...
        /// <summary>
        /// Format NdefFormatable tag
        /// </summary>
        /// <param name="tagInfo"><see cref="ITagInfo"/></param>
        /// <exception cref="Exception"></exception>
        void FormatNdefFormatable(ITagInfo tagInfo)
        {
            if (tagInfo == null)
                throw new Exception(Configuration.Messages.NFCErrorMissingTagInfo);
            ...
            try
            {
                ...
                var ndefMessage = GetEmptyNdefMessage();
                ndefFormatable.Format(ndefMessage);
                var nTag = GetTagInfo(_currentTag, ndefMessage);
        ...
        ITagInfo GetTagInfo(Tag tag, NdefMessage ndefMessage = null)
        {
            ...
            var isNdefCompatible = ndef != null || isNdefFormatable;
